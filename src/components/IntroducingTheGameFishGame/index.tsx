...
import styles from "../IntroducingTheGameSlotCasino/IntroducingTheGame.module.css";
...
function IntroducingTheGameFishGame({ gamename }: { gamename: string }) {
  ...
  const getDataAvalible = async () => {
    try {
      const res = await gameService.GameAvalibleV2({
        gameTypes: [`${GameType.FH}`, `${GameType.FISHING}`],
        gpIds: [
          `${ProviderID.BigGame}`,
          `${ProviderID.CQ9}`,
          `${ProviderID.JDB}`,
  ...
  return (
    <div className="py-[20px] flex justify-center flex-col items-center w-full lg:w-[850px]">
      ...
      >
        ...
        <Link
          href={"/games/FishGame/RecommendGame"}
      ...
      <div className=" py-[20px] relative max-h-[300px] h-full">
        <Slider
          dots={true}
          infinite={true}
          speed={500}
          ...
          className="w-[850px]"
        >
          {listGameIntro && listGameIntro.slice(0,15).map((item, index) => (
            <IntroItem
              dataGame={item}
              key={`${index}ds`}
...
export default IntroducingTheGameFishGame;
