...
import { listOptionsAgentBank } from "@/constant/ListOptionsAgentBank";
import { proposePriceBank } from "@/constant/proposePrice";
import { IInfo, IItemBankIn } from "@/type";
import { useQuery } from "@tanstack/react-query";
import { ridrectBankUrl } from "@/utils/check";

function BankPort({ info }: { info: undefined | IInfo }) {
  ...
  // const [dataBankIn, setDataBankIn] = useState<undefined | IItemBankIn[]>(
  //   undefined
  // );
  // const getListBankIn = async () => {
  //   try {
  //     const res = await paymentService.getListBankIn();
  ...
  const minMoney =
    listOptionsAgentBank.find((data) => data.id === bankSelected)?.min || 20;

  const maxMoney =
    listOptionsAgentBank.find((data) => data.id === bankSelected)?.max ||
    350000;

  const selectOption =
    bankSelected &&
    listOptionsAgentBank.find((item) => item.id === bankSelected);
  ...
  }, [inputValueToVnd, isError, hasError, bankValue, selectOption]);
      ...
      <div className="hidden md:block font-helvetica">
        <div className={styles.container}>
          <div className={styles.listBank}>
            {listOptionsAgentBank.map((item, index) => (
              <div
